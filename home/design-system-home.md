...
- Mobile (`≤768px`): `.testimonial-slide` pasa a columna
...
**Caso aparte — tipografía (`h1`-`h4`, `.text-body`, `.text-body--sm`, `.btn`):** el problema real ahí no era de orden sino de **especificidad**. Selectores de página como `.section--hero-home h1`, `.intro-home-text h2`, `.section--mentor-home h2`, `.section--apart-home h2/h3` tienen más especificidad (clase + elemento) que un simple `h1`/`h2`/`h3`, así que le ganaban a la regla mobile sin importar el orden. Se agregó `!important` en el bloque mobile para garantizar que siempre gane, sin importar qué tan específico sea un selector de página que la toque. Para `h1`-`h4` **solo se fuerza el `font-size`** (línea/peso/letter-spacing de cada H quedan a cargo del selector de página correspondiente, no se tocan desde el bloque mobile genérico).
