...
- **`.dropdown-content`** — usa la técnica CSS `grid-template-rows: 0fr → 1fr` (con `overflow: hidden` en el propio grid container **y** en `.dropdown-content-inner { min-height: 0; overflow: hidden }`) para animar la altura sin JS midiendo `scrollHeight` — se adapta solo a cualquier contenido que se le ponga adentro, sin recalcular nada. El `overflow: hidden` en `.dropdown-content` es necesario: sin él, el contenido real (con su propia altura por el texto) se desborda visualmente fuera del track colapsado a 0fr, dejando un "sliver" del párrafo visible con el dropdown cerrado.
