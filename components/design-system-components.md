...
El link "Work" del `.nav-links` desktop se reemplazó por un dropdown: `.nav-dropdown` (wrapper, `position:relative`) → `.nav-dropdown-toggle` (botón con texto "Work" + chevron `.nav-dropdown-chevron`, rota 180° cuando `.nav-dropdown.is-open`) → `.nav-dropdown-content` (panel absoluto, oculto por default vía `opacity:0`/`pointer-events:none`, visible con `.is-open`) con dos `.nav-dropdown-link` ("by projects" / "by clients") separados por un `.nav-dropdown-divider` real (no `border-bottom` en el link — así el divisor respeta el mismo `gap` del content en vez de crear doble espaciado).

**El panel es siempre el mismo, sin importar la variante activa del nav** (white/black/scrolled): bg blanco, texto negro, `padding: 1em`, `gap: 0.5em` entre los links/divider, `border: 1px solid rgba(0,0,0,.1)` (`#0000001A`) y un shadow sutil. No cambia de color con la variante — a diferencia del resto del nav, este panel es fijo. `.nav-dropdown-link` es un `<a>` dentro de `.nav-links`, así que sin cuidado `.nav--scrolled .nav-links a { color: white }` también le pegaría — por eso `.nav-dropdown-link { color: var(--color-black) !important; }`, para blindarlo de cualquier override de variante (incluido el estado `:hover`, que no define color propio y heredaría del selector con mayor especificidad si no fuera por el `!important`).
...
- **`.nav-hamburger`** — botón que contiene un ícono SVG estático de 3 líneas horizontales (`.nav-hamburger-icon`). Ya **no tiene animación** (se quitó lottie por completo — al abrir el modal fullscreen tapa el botón casi de inmediato, así que la morfología hamburger→X casi no se llegaba a ver). Usa `stroke="currentColor"`, hereda color de la variante del nav igual que el resto del contenido (ya no necesita el hack de `filter: invert()`).

**`.nav-mobile-menu` — modal fullscreen**: `position: fixed; inset: 0; z-index: 500`, siempre bg blanco/texto negro, independiente de la posición y variante del `.nav`. Oculto por defecto (`opacity: 0; pointer-events: none`), visible con `.is-open` (fade simple).

- **`.nav-mobile-header`** — alineado con el `.nav-container` colapsado: el padding del modal (`1.15em 5.25em 3.25em`) reconstruye la suma exacta del padding de `.nav` (2em en mobile) + `.nav-container` (3.25em) horizontal, y el `1.15em` vertical de `.nav-container`, para que el logo no "salte" de posición al abrir el modal. `.nav-mobile-logo img` es `height: 1.71em` — mismo tamaño que `.nav-logo img` del header real.
- **`.nav-mobile-close`** — botón 2.5em × 2.5em (mismo tamaño que `.nav-hamburger`), con un ícono tipo asterisco (`*`): 3 líneas de `stroke-width: 2` (2px) y ~14px de largo cada una, rotadas a 0°/45°/135° sobre el mismo centro (vía `transform="rotate(...)"` en cada `<line>` del SVG) — no es una X de 2 líneas, son 3.
- **`.nav-mobile-links`** — lista en columna. Cada item (`.nav-mobile-link` o el grupo `.nav-mobile-group`) usa `border-top: 1px solid rgba(0,0,0,.1)` como divisor (mismo patrón que `.dropdown-toggle` del componente `dropdown`), excepto el primero. `padding: 20px 0` fijo por item (px, no em — igual que la tipografía). Links (`.nav-mobile-link`): Inter, `font-weight: 500`, `font-size: 24px` fijo, `line-height: 160%`, `letter-spacing: 0`.
- **"Work" como acordeón inline** (`.nav-mobile-group`): `.nav-mobile-group-toggle` (misma tipografía que `.nav-mobile-link` — Inter 500, 24px — + `.nav-mobile-group-chevron`, rota 180° con `.is-open`; su `:hover` se queda en `font-weight: 500`, no pasa a bold) → `.nav-mobile-group-content` (mismo approach de altura medida por JS que `/components/dropdown`: `height: 0 → scrollHeight px → auto`) con `.nav-mobile-group-inner` conteniendo los `.nav-mobile-sublink` ("by projects" / "by Clients", `font-size: 14px` fijo, indentados con `padding-left`).
- El `.btn-nav` ("Let's Talk!") se repite dentro del modal, `font-size: 14px` fijo, forzado a su estilo outline negro por defecto (no hereda el color de la variante del nav, ya que el fondo del modal es siempre blanco). El layout general del modal es `justify-content: space-between` (header arriba, links justo debajo, CTA empujado al fondo).
- En mobile (`<=991px`), `.nav` cambia su padding lateral a `2em` (18px @ base 9px).

⚠️ El asset `hamburger-menu-animation.json` (en esta misma carpeta) quedó sin usar — no se borró por si se retoma la animación más adelante, pero `navbar.js`/`navbar.html` ya no lo referencian ni cargan `lottie-web`.
...
- **`.service-card-title`** — Lora, 500, `1.5em` (21px), line-height 123%. Mobile (`<=768px`): `font-size: 22px` fijo (regla global: en mobile la tipografía va en px, no em).
- **`.service-card-footer`** — fila `justify-content: space-between; align-items: end`: `.service-card-icon` (5em × 5em, **intercambiable** — el `<svg>` interno usa `fill="currentColor"` o `stroke="currentColor"` para heredar color automáticamente) + `.service-card-arrow`. Mobile (`<=768px`): ícono a `7em × 7em`.
...
- **`.dropdown-content-inner`** — `width: 90%` del padre (según pedido), `padding-bottom: 1.79em` (asunción, no especificado — ajustar si hace falta), `display: flex; flex-direction: column`. Puede contener **cualquier cosa** (párrafos, botones, listas, etc.).
- **`.dropdown-content-inner p`** — único elemento con estilo fijo garantizado: Inter, 400, `1.15em`/16px, line-height 160%, color blanco. Mobile (`<=768px`): `font-size: 13px` fijo (regla global: en mobile la tipografía va en px, no em). Cualquier otro elemento (botones, etc.) no tiene estilos propios del componente — se estiliza aparte según la página.
